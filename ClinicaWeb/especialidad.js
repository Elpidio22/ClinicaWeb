...
especialidadRouter.get("/", async (req, res) => {
  ...
  especialidadRouter.delete("/:id", async (req, res) => {
    const id = req.params.id;
    await db.execute("delete from especialidad where id_especialidad=:id", { id });
    res.status(200).send({ mensaje: "Especialidad eliminada" });
  });
